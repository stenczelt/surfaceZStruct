...
def generateIsomerPair_1(
    reactiveIndices_1, surfaceReactiveIndices, numOfSlabAtoms, numOfAds1Atoms
):
    # check reactive indices are <= than number of atoms
    assert (
        reactiveIndices_1[0] <= numOfAds1Atoms
        and reactiveIndices_1[1] <= numOfAds1Atoms
    )
    assert (
        surfaceReactiveIndices[0] <= numOfSlabAtoms
        and surfaceReactiveIndices[1] <= numOfSlabAtoms
    )
    listOfAdds = []
...
# creates a list of lists. Each list has indices of two atoms that wil be added
def generateIsomerPair_2(
    reactiveIndices_1, reactiveIndices_2, numOfSlabAtoms, numOfAds1Atoms, numOfAds2Atoms
):
    # check reactive indices are <= than number of atoms
    assert (
        reactiveIndices_1[0] <= numOfAds1Atoms
    ...
    assert (
        reactiveIndices_2[0] <= numOfAds2Atoms
        and reactiveIndices_2[1] <= numOfAds2Atoms
    )
    listOfAdds = []
    for index_1 in reactiveIndices_1:
...
def generate2IsomerPairs(
    ...
    if reactionType == 1:
        pairs = generateIsomerPair_1(
            reactiveIndices_1, reactiveIndices_2, numOfSlabAtoms, numOfAds1Atoms
        )
    else:
        pairs = generateIsomerPair_2(
            reactiveIndices_1,
            reactiveIndices_2,
            numOfSlabAtoms,
            numOfAds1Atoms,
...
def areConnected(inputFile, twoIndices):
    ...
    if (
        adsorbate.GetAtom(index_1).GetType()[0] == "H"
        and adsorbate.GetAtom(index_2).GetType()[0] == "H"
    ):
...
def findSlabSize(inFile, numOfSlabAtoms):
    ...
    for i in range(2, len(lines)):
        if (
            ...
            and float(lines[i].split()[3]) >= zCoordinate - TOLERANCE
        ):
            # atoms in the same z layer
            if (
                float(lines[i].split()[2]) <= yCoordinate + TOLERANCE
                and float(lines[i].split()[2]) >= yCoordinate - TOLERANCE
            ):
                numOfAtomsInX += 1
            ...
            if (
                ...
                or newYcoordinate <= currentYcoordinate - TOLERANCE
            ):
                numOfAtomsInY += 1
...
def main():
    ...
    if reactionType == 1 and numOfAdsorbates == 1:
        if len(surfaceReactiveIndices) < 1 or surfaceReactiveIndices[0] == 0:
            print(
                "ERROR: For inter-molecular reaction with one added adsorbate\
    ...
    for line in slab_lines:
        if "slab_in" in line:
            if "#" not in line:
                ...
                slab_size_z = int(
                    line.split("=")[2].split("(")[1].split(",")[2].split(")")[0]
                )
                slab_vac = float(line.split("=")[3].split(")")[0])
                slab = eval(slab_type)(
                    slab_atom,
                    size=(slab_size_x, slab_size_y, slab_size_z),
                    vacuum=slab_vac,
                )
    ...
    for file in files:
        ...
        if numOfAdsorbates == 1 and reactionType == 0:
            ...
            if addMoves != 2:
                print(
                    "ERROR: You need at least 2 add moves for dissociation of one adsorbate."
                )
                exit(-1)
            ...
            for element in breakCombos:
                ...
                if connected == True:
                    ...
                    listOfSitesFrag_1 = findNearbySites(
                        slab, indexIn, "bindingSites.xyz"
                    )
                    indexIn = element[1] + numOfSlabAtoms
                    listOfSitesFrag_2 = findNearbySites(
                        slab, indexIn, "bindingSites.xyz"
                    )
                    # find two sites that are farthest from two lists of binding sites
                    # TODO make sure the site is empty
                    farthestSites = findFarthestIndices(
                        listOfSitesFrag_1, listOfSitesFrag_2, "bindingSites.xyz"
                    )
                    if len(farthestSites) > 0:
                        ...
                        fh.write(
                            "BREAK " + str(firstNum) + "  " + str(secondNum) + "\n"
                        )
                        fh.write(
                            "ADD   "
                            + str(
                                farthestSites[0] + numOfSlabAtoms + numOfAds1Atoms + 1
                            )
                            + "  "
                            + str(firstNum)
                            + "\n"
                        ...
                        fh.write(
                            "ADD   "
                            + str(
                                farthestSites[1] + numOfSlabAtoms + numOfAds1Atoms + 1
                            )
                            + "  "
                            + str(secondNum)
                            + "\n"
                        ...
                        index = i
                        createTemplateFiles(
                            file, folder, index, slab, slabType, reactionName
                        )
                        ...
                        print(
                            "No binding sites found. Skipping ",
                            file.split(".")[0] + str(i).zfill(4),
                        )
                        i += 1
        ...
        # TODO ligand transfer
            ...
            if addMoves != 2 or breakMoves != 2:
                print(
                    "ERROR: You need at least 2 add and 2 break moves for this type of reaction."
                )
                exit(-1)
            if len(reactiveIndices_1) < 2 or len(surfaceReactiveIndices) < 2:
                print(
                    "ERROR: You need at least 2 reactive atoms on each adsorbate for ligand exchange."
                )
                exit(-1)
            ...
            for i in range(0, len(surfaceReactiveIndices)):
                for j in range(i + 1, len(surfaceReactiveIndices)):
                    if (surfaceReactiveIndices[i] != 0) and (
                        surfaceReactiveIndices[j] != 0
                    ):
                        temp = []
            ...
            for element_1 in breakCombos_1:
                for element_2 in breakCombos_surface:
                    ...
                    if connected_1 == True and connected_2 == True:
                        numOfAds2Atoms = 0
                        twoPairsSet = generate2IsomerPairs(
                            element_1,
                            element_2,
                            numOfSlabAtoms,
                            numOfAds1Atoms,
                        ...
                        for kk in range(0, len(twoPairsSet)):
                            ...
                            fh.write(
                                "ADD   "
                                + str(twoPairsSet[kk][0][0])
                                + "  "
                                + str(twoPairsSet[kk][0][1])
                                + "\n"
                            )
                            fh.write(
                                "ADD   "
                                + str(twoPairsSet[kk][1][0])
                                + "  "
                                + str(twoPairsSet[kk][1][1])
                                + "\n"
                            )
                            fh.close()
        ...
        ###############################################
            ...
            for element_1 in breakCombos_1:
                ...
                if connected_1 == True:
                    ...
                    for kk in range(0, len(pairsSet)):
                        ...
                        coordNum = getCoordinationNum(
                            adsorbFile2,
                            (pairsSet[kk][1] - numOfSlabAtoms - numOfAds1Atoms),
                        )
                        ...
                        # CoordinationNumbers()
                        MAX_COORDINATION_NUM = CoordinationNumbers().getMaxCoordNum(
                            atomicNumber
                        )
            ...
            for element_2 in breakCombos_2:
                ...
                if connected_2 == True:
                    ...
                    for kk in range(0, len(pairsSet)):
                        # check coordination number
                        coordNum = getCoordinationNum(
                            adsorbFile1, (pairsSet[kk][0] - numOfSlabAtoms)
                        )
                        atomicNumber = getAtomicNumber(
                            adsorbFile1, (pairsSet[kk][0] - numOfSlabAtoms)
                        )
            ...
            if addMoves != 2:
                if breakMoves != 2:
                    print(
                        "ERROR: You need at least 2 add and break moves for ligand exchange."
                    )
                    exit(-1)
