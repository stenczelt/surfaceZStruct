...
void RXNDB::get_rowX_nbo(int wg, int wr, double* X, int sort)
{
...
void RXNDB::save_xy_data(string filename)
{
 ...
 //reset any modifications
  ...
  for (int i=0;i<N;i++)
  if (unique[i])
    get_rowX_nbo(wg,i,&X[i*dim],sortme);
  ...
  if (unique[i])
  {
    ...
    for (int j=0;j<nadd[i];j++)
    {
      sprintf(sbuff," %s-%s",anames[i][add[i][2*j+0]].c_str(),anames[i][add[i][2*j+1]].c_str()); xyfile << sbuff;
    }
    sprintf(sbuff,"\n   brk:"); xyfile << sbuff;
    for (int j=0;j<nbrks[i];j++)
    {
      sprintf(sbuff," %s-%s",anames[i][brks[i][2*j+0]].c_str(),anames[i][brks[i][2*j+1]].c_str()); xyfile << sbuff;
    }
    sprintf(sbuff,"\n "); xyfile << sbuff;
    for (int j=0;j<dim;j++)
    {
